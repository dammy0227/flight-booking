...
import { GiCommercialAirplane, GiAirplaneArrival, GiAirplaneDeparture, GiAirplane } from 'react-icons/gi';
import { TbPlane, TbPlaneInflight } from 'react-icons/tb';
...
const UserFlights = () => {
  ...
  const filteredFlights = flights?.filter(flight => {
    ...
    if (searchParams.from && flight.departureCity !== searchParams.from) return false;
    if (searchParams.to && flight.arrivalCity !== searchParams.to) return false;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-10">
        <div className="space-y-4 sm:space-y-6">
          {/* Header */}
          <div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800">Flights</h1>
            <p className="text-gray-500 text-xs sm:text-sm mt-1">Find and book the best flights for your journey</p>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-10 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-800 text-sm focus:outline-none focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C]/50 transition-colors appearance-none cursor-pointer"
                  >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-800 text-sm focus:outline-none focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C]/50 transition-colors cursor-pointer"
                  />
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none px-6 py-2 bg-[#C9A84C] rounded-xl text-white font-semibold hover:bg-[#B8922E] transition-colors flex items-center justify-center gap-2 shadow-sm"
                  >
                    <FiSearch size={16} />
              ...
              {showFilters && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                    <div>
                      <label className="text-gray-600 text-sm mb-2 block">Max Price</label>
                    ...
                    <div>
                      <label className="text-gray-600 text-sm mb-2 block">Minimum Seats</label>
                      <select
          ...
          {loading ? (
            <div className={`grid ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} gap-4 sm:gap-5`}>
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="bg-gray-50 rounded-xl border border-gray-100 p-4 sm:p-5 animate-pulse">
            ...
            <div className={`grid ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} gap-4 sm:gap-5`}>
              {sortedFlights.map((flight) => (
                viewMode === 'grid' ? (
                  // Grid View Card - Mobile Responsive
                  <div 
                    key={flight._id} 
                    className="group bg-white rounded-xl border border-gray-100 hover:border-[#C9A84C]/40 hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col"
                  >
                      ...
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-gray-800 font-bold text-sm sm:text-base truncate">
                        ...
                        <div className="text-right shrink-0">
                          <p className="text-[#C9A84C] text-lg sm:text-xl font-bold">${flight.price}</p>
                          <p className={`text-[10px] sm:text-xs font-semibold ${flight.availableSeats < 10 ? 'text-amber-500' : 'text-emerald-500'}`}>
                    ...
                    <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <p className="text-gray-800 font-bold text-base sm:text-lg">{formatTime(flight.departureTime)}</p>
                          <p className="text-gray-400 text-[10px] sm:text-xs mt-0.5">{formatDate(flight.departureTime)}</p>
                          <p className="text-gray-500 text-[10px] sm:text-xs font-medium mt-0.5 truncate">{flight.departureCity}</p>
                        </div>
                        <div className="flex flex-col items-center px-2 sm:px-3">
                          <p className="text-gray-400 text-[10px] sm:text-xs font-medium">{calculateDuration(flight.departureTime, flight.arrivalTime)}</p>
                          <div className="relative w-12 sm:w-16 my-1 sm:my-2">
                            ...
                            <span className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-0.5 sm:px-1 text-[#C9A84C] text-[8px] sm:text-xs">
                              <FaPlane size={8} />
                            </span>
                          </div>
                          <p className="text-gray-400 text-[10px] sm:text-xs">Direct</p>
                        </div>
                        <div className="flex-1 text-right">
                          <p className="text-gray-800 font-bold text-base sm:text-lg">{formatTime(flight.arrivalTime)}</p>
                          <p className="text-gray-400 text-[10px] sm:text-xs mt-0.5">{formatDate(flight.arrivalTime)}</p>
                          <p className="text-gray-500 text-[10px] sm:text-xs font-medium mt-0.5 truncate">{flight.arrivalCity}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-3 sm:gap-4 pt-2 border-t border-gray-100">
                        <div className="flex items-center gap-1 sm:gap-2">
                          <MdAirlineSeatReclineNormal size={12} className="text-gray-400" />
                          <span className="text-gray-500 text-[10px] sm:text-xs">{flight.availableSeats} seats</span>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2">
                          <FaClock size={12} className="text-gray-400" />
                          <span className="text-gray-500 text-[10px] sm:text-xs">{calculateDuration(flight.departureTime, flight.arrivalTime)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 sm:p-5 pt-0 flex gap-2">
                      <button
                        ...
                        className="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 border border-[#C9A84C]/40 rounded-xl text-[#C9A84C] text-xs sm:text-sm font-semibold hover:bg-[#C9A84C]/10 transition-colors flex items-center justify-center gap-1 sm:gap-2"
                      >
                        <FiInfo size={12} />
                        Details
                      </button>
                      <button
                        ...
                        className={`flex-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold transition-colors flex items-center justify-center gap-1 sm:gap-2 ${
                  ...
                  // List View Card - Mobile Responsive
                  <div 
                    key={flight._id} 
                    className="group bg-white rounded-xl border border-gray-100 hover:border-[#C9A84C]/40 hover:shadow-lg transition-all duration-300 overflow-hidden"
                  >
                    <div className="p-4 sm:p-5">
                      <div className="flex flex-col gap-4">
                        <div className="flex items-start gap-3 sm:gap-4">
                          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-[#C9A84C]/10 flex items-center justify-center shrink-0">
                            {getAirlineIcon(flight.airline)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                              ...
                              <div className="text-right">
                                <p className="text-[#C9A84C] text-xl sm:text-2xl font-bold">${flight.price}</p>
                                <p className={`text-xs sm:text-sm font-semibold ${flight.availableSeats < 10 ? 'text-amber-500' : 'text-emerald-500'}`}>
                            ...
                            <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <p className="text-gray-800 font-semibold text-xs sm:text-sm">{formatTime(flight.departureTime)}</p>
                                  <p className="text-gray-400 text-[10px] sm:text-xs">{formatDate(flight.departureTime)}</p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <MdFlightLand className="text-[#C9A84C]" size={14} />
                                <div>
                                  <p className="text-gray-800 font-semibold text-xs sm:text-sm">{formatTime(flight.arrivalTime)}</p>
                                  <p className="text-gray-400 text-[10px] sm:text-xs">{formatDate(flight.arrivalTime)}</p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <MdAccessTime className="text-[#C9A84C]" size={14} />
                                <p className="text-gray-500 text-xs sm:text-sm">{calculateDuration(flight.departureTime, flight.arrivalTime)}</p>
                              </div>
                            </div>
                            ...
                            <div className="flex gap-2 sm:gap-3 mt-3">
                              <button
                                onClick={() => handleViewDetails(flight)}
                                className="px-3 sm:px-4 py-1.5 sm:py-2 border border-[#C9A84C]/40 rounded-xl text-[#C9A84C] text-xs sm:text-sm font-semibold hover:bg-[#C9A84C]/10 transition-colors"
                              >
                                View Details
                              </button>
                              <button
                                onClick={() => handleBookFlight(flight)}
                                disabled={flight.availableSeats < searchParams.passengers}
                                className={`flex-1 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${
                                  flight.availableSeats >= searchParams.passengers
                                    ? 'bg-[#C9A84C] text-white hover:bg-[#B8922E]'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                }`}
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-8 sm:p-12 text-center">
              <div className="text-5xl sm:text-6xl mb-4 flex justify-center">
                <GiAirplane size={50} className="text-[#C9A84C]" />
              </div>
              <h3 className="text-gray-800 text-lg sm:text-xl font-semibold mb-2">No flights found</h3>
              <p className="text-gray-500 text-xs sm:text-sm mb-5">Try adjusting your search criteria or filters</p>
              <button
                onClick={handleResetFilters}
                className="px-5 sm:px-6 py-2 sm:py-2.5 bg-[#C9A84C] rounded-xl text-white text-sm font-semibold hover:bg-[#B8922E] transition-colors"
              >
                Clear All Filters
              </button>
            </div>
