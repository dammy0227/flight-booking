...
import { loginUser, firebaseLogin, logoutUser } from '../../features/users/userSlice';
...
const Login = () => {
  ...
  const [adminBlockError, setAdminBlockError] = useState('');

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
    if (errors[name]) setErrors(prev => ({ ...prev, [name]: '' }));
    if (adminBlockError) setAdminBlockError('');
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!firebaseUser || !firebaseUser.token) throw new Error('Google sign-in failed');
      ...
      if (result.user.role === 'admin') {
        dispatch(logoutUser());
        setAdminBlockError('Admin accounts must sign in via the Admin Portal.');
        return;
      }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-6 pt-8">
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 text-[#C9A84C] mb-2"><FiGlobe size={24} /></div>
                <p className="text-2xl font-bold text-[#F5F0E8]">150+</p>
                <p className="text-[#8B92A5] text-sm">Destinations</p>
              </div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 text-[#C9A84C] mb-2"><FiUsers size={24} /></div>
                <p className="text-2xl font-bold text-[#F5F0E8]">50K+</p>
                <p className="text-[#8B92A5] text-sm">Happy Travelers</p>
              </div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 text-[#C9A84C] mb-2"><FiAward size={24} /></div>
          ...
          <div className="flex items-center gap-6 text-sm">
            <div className="flex items-center gap-2 text-[#8B92A5]"><FiShield size={16} /><span>Secure Login</span></div>
            <div className="w-px h-4 bg-[#252E44]"></div>
            <div className="flex items-center gap-2 text-[#8B92A5]"><FiStar size={16} /><span>4.9/5 Rating</span></div>
            <div className="w-px h-4 bg-[#252E44]"></div>
            <div className="flex items-center gap-2 text-[#8B92A5]"><FiLock size={16} /><span>256-bit SSL</span></div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-120">
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl p-8 md:p-10">
            <div className="mb-8">
              ...
              <h1 className="text-3xl md:text-4xl font-bold text-[#F5F0E8] mb-2">Sign in</h1>
              <p className="text-[#8B92A5]">Access your account to manage bookings</p>
            </div>

            {/* Admin block error */}
            {adminBlockError && (
              <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
                <p className="text-amber-400 text-sm font-medium">{adminBlockError}</p>
                <Link
                  to="/admin/login"
                  className="inline-flex items-center gap-1 text-[#C9A84C] text-sm font-semibold mt-2 hover:opacity-80 transition-opacity"
                >
                  Go to Admin Portal <FiChevronRight size={14} />
                </Link>
              </div>
            )}

            {/* Generic API error */}
            {error && !adminBlockError && (
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                <label className="block text-[#8B92A5] text-sm font-medium mb-2">Email Address</label>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[#8B92A5] text-sm font-medium">Password</label>
                  <button type="button" onClick={handleForgotPassword} className="text-[#C9A84C] text-sm hover:opacity-80 transition-opacity">
                    Forgot password?
                  </button>
                </div>
                <div className="relative">
              ...
              <div className="text-center pt-4">
                <p className="text-[#8B92A5]">
                  ...
                  <Link to="/register" className="text-[#C9A84C] font-semibold hover:opacity-80 transition-opacity inline-flex items-center gap-1">
                    Create account <FiChevronRight size={16} />
                  </Link>
                </p>
              </div>
            </form>
          ...
          <div className="lg:hidden mt-6 flex justify-center gap-6 text-xs text-[#8B92A5]">
            <div className="flex items-center gap-2"><FiShield size={14} /><span>Secure Login</span></div>
            <div className="w-px h-3 bg-[#252E44]"></div>
            <div className="flex items-center gap-2"><FiStar size={14} /><span>4.9 Rating</span></div>
            <div className="w-px h-3 bg-[#252E44]"></div>
            <div className="flex items-center gap-2"><FiLock size={14} /><span>SSL Secure</span></div>
          </div>
        </div>
      </div>
    </div>
