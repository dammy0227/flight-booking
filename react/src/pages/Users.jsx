      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="mb-6 sm:mb-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-6">
            {[
              { label: "Total Users", value: userStats.total, icon: <FiUser className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" />, bg: "bg-blue-100" },
              { label: "Active Users", value: userStats.active, icon: <FiUserCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-600" />, bg: "bg-green-100" },
              { label: "Inactive Users", value: userStats.inactive, icon: <FiUserX className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600" />, bg: "bg-gray-100" },
              { label: "New This Week", value: userStats.newThisWeek, icon: <FiClock className="w-4 h-4 sm:w-5 sm:h-5 text-orange-600" />, bg: "bg-orange-100" },
            ].map(({ label, value, icon, bg }) => (
              <div key={label} className="bg-white rounded-xl p-3 sm:p-6 border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs sm:text-sm text-gray-500">{label}</p>
                    <p className="text-xl sm:text-2xl font-semibold text-gray-900 mt-1">{value}</p>
                  </div>
                  <div className={`w-8 h-8 sm:w-10 sm:h-10 ${bg} rounded-lg flex items-center justify-center`}>
                    {icon}
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 mb-6">
          <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-9 sm:pl-10 pr-4 py-2 text-sm sm:text-base border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <select
                ...
                className="flex-1 sm:flex-none px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 bg-white"
              >
          ...
          {selectedUsers.length > 0 && (
            <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="flex space-x-2 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none px-3 py-1.5 text-sm bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-colors"
                >
                  Make Admin
                </button>
                <button
        ...
        {!loading && filteredUsers.length === 0 ? (
          <div className="text-center py-12 sm:py-20 bg-white rounded-xl border border-gray-200">
            <FiUser className="mx-auto w-10 h-10 sm:w-12 sm:h-12 text-gray-400" />
            <h3 className="mt-4 text-base sm:text-lg font-medium text-gray-900">No users found</h3>
            <p className="mt-1 text-sm text-gray-500">
                ...
                <div className="p-4 sm:p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3 sm:space-x-4 flex-1">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full ${getRandomColor(
                          user.name
                        )} flex items-center justify-center text-white font-semibold text-base sm:text-lg shrink-0`}
                      >
                        {user.image ? (
                          <img
                            src={user.image}
                        ...
                        <div className="flex items-center">
                          <div
                            className={`w-8 h-8 rounded-full ${getRandomColor(
                              user.name
                            )} flex items-center justify-center text-white font-semibold text-sm mr-3 shrink-0`}
                          >
                            {user.image ? (
                              <img
                                src={user.image}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      <input
                        ...
                        checked={selectedUsers.includes(user._id)}
                        onChange={() => handleSelectUser(user._id)}
                        className="w-5 h-5 text-orange-600 rounded border-gray-300 focus:ring-orange-500 shrink-0"
                      />
                      <div
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-4 sm:p-6">
                <div className="flex items-center space-x-4 mb-6">
                  <div
                    className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full ${getRandomColor(
                      showUserDetails.name
                    )} flex items-center justify-center text-white font-semibold text-lg sm:text-xl shrink-0`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 wrap-break-word">{showUserDetails.name}</h3>
                    <p className="text-sm text-gray-500 wrap-break-word">{showUserDetails.email}</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    <p className="text-xs sm:text-sm text-gray-500 mb-1">User ID</p>
                    <p className="font-mono text-xs sm:text-sm text-gray-900 wrap-break-word">{showUserDetails._id}</p>
                  </div>
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    <p className="text-xs sm:text-sm text-gray-500 mb-1">Role</p>
                    <p className="font-medium text-sm sm:text-base text-gray-900 capitalize">{showUserDetails.role}</p>
                  </div>
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                  ...
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    ...
                    <p className={`font-medium text-sm sm:text-base ${showUserDetails.isActive ? "text-green-600" : "text-gray-400"}`}>
              ...
              <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 p-4 sm:p-6 border-t border-gray-200 sticky bottom-0 bg-white">
                <button
                  onClick={() => setShowUserDetails(null)}
                  className="px-4 py-2 border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 order-2 sm:order-1"
                >
                  Close
                </button>
                <button
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-4 sm:p-6">
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-red-100 rounded-full mx-auto mb-4">
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 order-2 sm:order-1"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDelete}
                  className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg order-1 sm:order-2"
                >
                  Delete
                </button>
              </div>
        ...
        {showEditRoleModal && userToEdit && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-4 sm:p-6">
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-purple-100 rounded-full mx-auto mb-4">
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-4">
                  ...
                  >
                    <FiUser className={`w-5 h-5 sm:w-6 sm:h-6 mx-auto mb-2 ${newRole === "user" ? "text-green-600" : "text-gray-400"}`} />
                  ...
                  >
                    <FiShield className={`w-5 h-5 sm:w-6 sm:h-6 mx-auto mb-2 ${newRole === "admin" ? "text-purple-600" : "text-gray-400"}`} />
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 pt-4">
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-2.5 rounded-lg text-white order-1 sm:order-2 ${
